...
def create_session(session: WorkoutSessionCreate, db: DB):
    db_session = WorkoutSession(**session.model_dump())
    db.add(db_session)
    db.commit()
...
def get_sessions(db: DB, skip: int = 0, limit: int = 100):
...
def update_session(session_id: int, payload: WorkoutSessionCreate, db: DB):
    session = db.query(WorkoutSession).filter(WorkoutSession.Session_ID == session_id).first()
    if not session:
        raise HTTPException(404, "Workout session not found")
